    ...
    public static class Miles
    {
        ...
        public static List<Mile> GetClubhouseName(string searchString, WMMCRC database)
        {
            ...
            var milesListObj = (from a in database.Miles
                                ...
                                select new { a.MilesId, a.FromId, a.ToId, a.Miles, a.Active, a.DateModified, a.DateCreated,a.GoogleUri }).ToList();
            
            List<Mile> milesList = new List<Mile>();
            foreach(var item in milesListObj)
            {
                milesList.Add(new Mile (item.MilesId, item.FromId, item.ToId, item.Miles, item.Active, item.DateModified, item.DateCreated,item.GoogleUri));
            }
